...
class OrdersHistoryHandler implements HandlerInterface
{
    public function prepare($data)
    {
        ...
        foreach($data as $record) {
			...
			echo "OrdersHistoryHandler"; print_r($record);
            ...
            if (empty($record['externalId'])) {
				...
				if (!empty($record['delivery']['code'])) $tmp_sql[]="delivery_id='".$record['delivery']['code']."'";
				if (!empty($record['paymentType'])) $tmp_sql[]="payment_method_id='".$record['paymentType']."'";
				if (!empty($record['paymentStatus']) && $record['paymentStatus'] == 'paid') $tmp_sql[]="paid='1'";
				if (!empty($record['paymentStatus']) && $record['paymentStatus'] == 'not-paid') $tmp_sql[]="paid='0'";
				if (!empty($record['isClosed'])) $tmp_sql[]="closed='".$record['isClosed']."'";
				if (!empty($record['createdAt'])) $tmp_sql[]="date='".$record['createdAt']."'";
				...
				if (!empty($record['email'])) $tmp_sql[]="email='".$record['email']."'";
				if (!empty($record['managerComment'])) $tmp_sql[]="comment='".$record['managerComment']."'";
				if (!empty($record['summ'])) $tmp_sql[]="total_price='".$record['summ']."'";
				if (!empty($record['discount'])) $tmp_sql[]="discount='".$record['discount']."'";
				if (!empty($record['managerComment'])) $tmp_sql[]="note='".$record['managerComment']."'";
				
				$this->sql = $this->container->db->prepare(
					'UPDATE `s_orders` SET '
						 . implode(", ", $tmp_sql) . '
					WHERE id=:externalId');
            ...
            if (!empty($record['items'])) {
                ...
                foreach($record['items'] as $item) {
					...
					if (!empty($item['deleted'])) {
						...
						"DELETE FROM `s_purchases` WHERE order_id='".$record['externalId']."' AND variant_id='".$item['id']."'"
						);
						$this->query->execute();
						continue;
					}
